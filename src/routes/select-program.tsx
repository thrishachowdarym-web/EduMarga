...
import { Sparkles, GraduationCap, BrainCircuit, ArrowRight, Loader2 } from "lucide-react";

import { NeuralBackground } from "@/components/NeuralBackground";

export const Route = createFileRoute("/select-program")({
  head: () => ({
    meta: [
      { title: "Choose Your Program | LearnPath AI" },
      {
        name: "description",
        content:
          "Select B.Tech or M.Tech to get a personalized AI-driven learning roadmap from LearnPath AI.",
      },
      { property: "og:title", content: "Choose Your Program | LearnPath AI" },
      {
  ...
  component: SelectProgramPage,
});

type Program = "btech" | "mtech";
...
const programs: {
  ...
  description: string;
  cta: string;
  to: "/branch-selection" | "/mtech-specialization";
}[] = [
  {
    id: "btech",
    title: "B.Tech",
    icon: GraduationCap,
    description: "Build your technical foundation and career roadmap.",
    cta: "Continue with B.Tech",
    to: "/branch-selection",
  },
  {
    id: "mtech",
    title: "M.Tech",
    icon: BrainCircuit,
    description: "Explore advanced learning, specialization and research paths.",
    cta: "Continue with M.Tech",
    to: "/mtech-specialization",
  },
];

function SelectProgramPage() {
  const navigate = useNavigate();
  const [pending, setPending] = useState<Program | null>(null);

  const select = (program: Program, to: "/branch-selection" | "/mtech-specialization") => {
    setPending(program);
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col items-center justify-center px-6 py-16">
        <header className="animate-rise-in text-center">
          ...
          <h1 className="mt-7 font-display text-4xl font-bold leading-[1.05] tracking-tight sm:text-5xl lg:text-6xl">
            <span className="text-gradient-brand">Choose Your Learning Path</span>
          </h1>

          <p className="mx-auto mt-5 max-w-xl text-base leading-relaxed text-muted-foreground sm:text-lg">
            Select your program to get a personalized roadmap.
          </p>
        </header>

        <div className="mt-14 grid w-full gap-7 sm:grid-cols-2 lg:gap-9">
          {programs.map(({ id, title, icon: Icon, description, cta, to }, index) => {
        ...
        <p className="mt-12 text-center text-xs text-muted-foreground">
          <Link to="/" className="font-medium text-cyan transition-opacity hover:opacity-80">
            Back to login
          </Link>
        </p>
      </div>
